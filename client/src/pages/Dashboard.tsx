...
export default function Dashboard() {
  ...
  return <MetrologicShell eyebrow="Enforcement command" title="Inspection overview" actions={<button onClick={() => navigate("/scan")} className="signal-button h-10 px-3"><Plus className="mr-2 h-3.5 w-3.5" />New scan</button>}>
    <div className="space-y-9">
      <section className="dashboard-reveal grid gap-5 lg:grid-cols-[1.4fr_.6fr]">
        <div className="ink-card relative min-h-[300px] overflow-hidden px-5 py-7 sm:px-8 sm:py-9">
          <div aria-hidden="true" className="dashboard-glow dashboard-glow--hero pointer-events-none absolute -right-20 -top-24 h-80 w-80 rounded-full" />
          <div aria-hidden="true" className="dashboard-orbit dashboard-orbit--hero pointer-events-none absolute -right-16 -top-20 h-72 w-72 rounded-full border border-[#ffd600]/35" />
          <div aria-hidden="true" className="dashboard-orbit dashboard-orbit--hero-reverse pointer-events-none absolute -right-3 top-8 h-52 w-52 rounded-full border border-[#ffd600]/25" />
          <div aria-hidden="true" className="dashboard-sweep pointer-events-none absolute inset-y-0 right-0 w-1/2" />
          <div aria-hidden="true" className="pointer-events-none absolute bottom-6 right-8 grid grid-cols-5 gap-2 opacity-60">{Array.from({ length: 15 }).map((_, index) => <span key={index} className={cn("dashboard-spark h-1 w-1", index % 4 === 0 ? "bg-[#ffd600]" : "bg-[#66705e]")} style={{ animationDelay: `${index * 0.18}s` }} />)}</div>
        ...
        <aside className="paper-card dashboard-reveal relative flex min-h-[300px] flex-col justify-between overflow-hidden p-5 sm:p-6">
          <div aria-hidden="true" className="dashboard-glow dashboard-glow--panel pointer-events-none absolute -right-10 -top-12 h-40 w-40 rounded-full" />
          <div aria-hidden="true" className="dashboard-orbit dashboard-orbit--panel pointer-events-none absolute -right-8 -top-8 h-28 w-28 rounded-full border border-[#ffd600]/25" />
          <div aria-hidden="true" className="dashboard-orbit dashboard-orbit--panel-reverse pointer-events-none absolute right-8 top-20 h-16 w-16 rounded-full border border-[#66705e]/30" />
          <div className="relative flex items-start justify-between"><div className="grid h-10 w-10 place-items-center bg-[#ffd600] text-[#11120f]"><ScanLine className="h-5 w-5" /></div><span className="font-mono text-[9px] uppercase tracking-[0.16em] text-[#777970]">Ready state</span></div>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.5fr_.65fr]">
        ...
        <aside className="dashboard-reveal ink-card motion-card relative overflow-hidden p-5 sm:p-6"><div aria-hidden="true" className="dashboard-glow dashboard-glow--rhythm pointer-events-none absolute -right-14 -top-14 h-44 w-44 rounded-full" /><div aria-hidden="true" className="dashboard-orbit dashboard-orbit--rhythm pointer-events-none absolute -right-8 -top-8 h-28 w-28 rounded-full border border-[#ffd600]/25" /><div aria-hidden="true" className="dashboard-orbit dashboard-orbit--rhythm-reverse pointer-events-none absolute right-10 top-14 h-14 w-14 rounded-full border border-[#66705e]/35" /><div className="relative"><p className="section-kicker text-[#ffd600]">Inspection rhythm</p><div className="mt-7 flex items-end gap-2"><span className="font-display text-6xl font-semibold leading-none tracking-[-0.08em] text-[#ffd600]">07</span><span className="mb-1 font-mono text-[9px] uppercase leading-relaxed tracking-[0.13em] text-[#b9baaf]">declaration<br />checks</span></div><div aria-hidden="true" className="mt-7 grid grid-cols-7 gap-1">{Array.from({ length: 7 }).map((_, index) => <span key={index} className={cn("dashboard-rhythm-bar h-2", index < 5 ? "bg-[#ffd600]" : "bg-[#42443b]")} style={{ animationDelay: `${index * 0.22}s` }} />)}</div><p className="mt-5 font-mono text-[10px] leading-relaxed text-[#b9baaf]">A compact readout for the rules that turn a label image into an inspection record.</p></div></aside>
      </section>
    </div>
  </MetrologicShell>;
}
