    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.16fr_.84fr]">
        <div className="scan-reveal paper-card motion-card overflow-hidden">
          ...
          <div className="border-t border-[#11120f]/15 p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><label className="section-kicker">Opening notes <span className="normal-case tracking-normal">(optional)</span></label><span className="font-mono text-[9px] uppercase tracking-[0.12em] text-[#9b9c92]">{notes.length} chars</span></div><Textarea value={notes} onChange={event => setNotes(event.target.value)} placeholder="Initial inspection context, location, or observed issue…" className="mt-3 min-h-24 resize-y rounded-none border-[#11120f]/20 bg-white font-mono text-xs focus-visible:ring-[#b29a00]" /><button onClick={runAnalysis} disabled={!staged.length || mutation.isPending} className="signal-button mt-4 flex h-11 w-full min-w-0 gap-2 px-3 disabled:cursor-not-allowed disabled:opacity-45">{mutation.isPending ? <LoaderCircle className="h-4 w-4 shrink-0 animate-spin" /> : <ScanLine className="h-4 w-4 shrink-0" />}<span className="min-w-0 truncate">{mutation.isPending ? "Gemini Vision is reading the label" : "Extract & inspect package"}</span><span className="ml-auto shrink-0 whitespace-nowrap text-[9px] opacity-60">{staged.length}/6</span></button></div>
        </div>
